...
Delete all existing inactive TOTP authenticators and create a new TOTP authenticator.
...
Activate the just created inactive TOTP authenticator.
...
Delete existing inactive OOB authenticators and create a new OOB authenticator.
If the phone or email has already been activated, Duplicated error is returned.
...
Activate the just created inactive OOB authenticator.
...
async function activateTOTP(otp: string): Promise<ActivateTOTPResult>;
...
async function activateOOB(code: string): Promise<ActivateOOBResult>;
...
const { recoveryCodes } = await skygear.auth.mfa.activateTOTP(otp);
```
...
const { recoveryCodes } = await skygear.auth.mfa.activateOOB(code);
```
